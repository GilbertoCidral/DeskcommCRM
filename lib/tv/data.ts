...
import type { TvData } from "@/app/app/tv/TvDashboard";

export const TV_VALID_WINDOWS = [6, 12, 24, 36, 60] as const;
export type TvWindowMonths = (typeof TV_VALID_WINDOWS)[number];
...
export async function fetchTvData(
  ...
  for (const r of (wonRaw ?? []) as Array<{ closed_at: string | null; value_cents: number | null }>) {
